...
MiniGrafx::MiniGrafx(DisplayDriver *driver, uint8_t bitsPerPixel, uint16_t
  *palette)
{
  this->driver = driver;
  ...
  {
    Serial.println(
      "[DEBUG_MINI_GRAFX][init] Not enough memory to create display\n");
  }
...
void MiniGrafx::fillRect(int16_t xMove, int16_t yMove, int16_t width, int16_t
  height)
{
...
void MiniGrafx::drawString(int16_t xMove, int16_t yMove, String strUser)
{
  ...
  while (textPart != NULL)
  {
    ...
    drawStringInternal(xMove, yMove - yOffset + (line++) *lineHeight, textPart,
      length, getStringWidth(textPart, length));
    textPart = strtok(NULL, "\n");
  }
  free(text);
}

void MiniGrafx::drawStringMaxWidth(int16_t xMove, int16_t yMove, uint16_t
  maxLineWidth, String strUser)
{
  ...
  {
    strWidth += pgm_read_byte(fontData + JUMPTABLE_START + (text[i] - firstChar)
      *JUMPTABLE_BYTES + JUMPTABLE_WIDTH);
    ...
    if (strWidth >= maxLineWidth)
    {
      ...
      drawStringInternal(xMove, yMove + (lineNumber++) *lineHeight,
        &text[lastDrawnPos], preferredBreakpoint - lastDrawnPos,
        widthAtBreakpoint);
  ...
  {
    drawStringInternal(xMove, yMove + lineNumber * lineHeight,
      &text[lastDrawnPos], length - lastDrawnPos, getStringWidth
      (&text[lastDrawnPos], length - lastDrawnPos));
  }

  free(text);
}

void MiniGrafx::drawStringInternal(int16_t xMove, int16_t yMove, char *text,
  uint16_t textLength, uint16_t textWidth)
{
  ...
  for (uint16_t j = 0; j < textLength; j++)
  {
    ...
    if (code >= firstChar)
    {
      ...
      byte msbJumpToChar = pgm_read_byte(fontData + JUMPTABLE_START + charCode
        * JUMPTABLE_BYTES); // MSB  \ JumpAddress
      byte lsbJumpToChar = pgm_read_byte(fontData + JUMPTABLE_START + charCode
        * JUMPTABLE_BYTES + JUMPTABLE_LSB); // LSB /
      byte charByteSize = pgm_read_byte(fontData + JUMPTABLE_START + charCode *
        JUMPTABLE_BYTES + JUMPTABLE_SIZE); // Size
      ...
      if (!(msbJumpToChar == 255 && lsbJumpToChar == 255))
      {
        ...
        drawInternal(xPos, yPos, currentCharWidth, textHeight, fontData,
          charDataPosition, charByteSize);
      }
...
uint16_t MiniGrafx::getStringWidth(const char *text, uint16_t length)
{
  ...
  while (length--)
  {
    stringWidth += pgm_read_byte(fontData + JUMPTABLE_START + (text[length] -
      firstChar) *JUMPTABLE_BYTES + JUMPTABLE_WIDTH);
    if (text[length] == 10)
    {
...
void inline MiniGrafx::drawInternal(int16_t xMove, int16_t yMove, int16_t width,
  int16_t height, const char *data, uint16_t offset, uint16_t bytesInData)
{
  if (width < 0 || height < 0)
    return ;
...
{
  if (x >= width || y >= height || x < 0 || y < 0 || color < 0 || color > 15 ||
...
void MiniGrafx::drawXbm(int16_t xMove, int16_t yMove, int16_t width, int16_t
...
void MiniGrafx::drawBmpFromFile(String filename, uint8_t x, uint16_t y)
{
  ...
  if (head->bfType == 0x4D42)
  {
    ...
    if (info->biPlanes == 1)
    {
      ...
      if ((bmpDepth == 24) && (info->biCompression == 0))
      {
        // 0 = uncompressed
...
void MiniGrafx::drawPalettedBitmapFromPgm(uint16_t xMove, uint16_t yMove, const
  char *palBmp)
{
...
void MiniGrafx::fillBottomFlatTriangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
void MiniGrafx::fillTopFlatTriangle(uint16_t x1, uint16_t y1, uint16_t x2,
  uint16_t y2, uint16_t x3, uint16_t y3)
{
...
void MiniGrafx::fillTriangle(uint16_t x1In, uint16_t y1In, uint16_t x2In,
  uint16_t y2In, uint16_t x3In, uint16_t y3In)
{
  ...
  else
  {
    ...
    int x4 = (int)(x[0] + ((float)(y[1] - y[0]) / (float)(y[2] - y[0]))*(x[2] -
      x[0]));
    int y4 = y[1];
...
void MiniGrafx::drawTriangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
